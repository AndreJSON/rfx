...
- The `+` button opens the recipe editor modal in "create" mode.

## Recipe List

- Each list item displays two lines: **title** (line 1) and comma-separated tags (line 2).
- Clicking a list item opens the **View Modal**.
- The list is sourced from the Pinia recipe store.

## Modals

### View Modal (read mode)
- Displays: title, tags, body text, image (if present).
- Controls: `×` (close) icon button, pen/edit icon button.
- Clicking the pen icon switches to Edit Modal.

### Edit Modal (edit/create mode)
...
- `uploadImage(id, file)` / `removeImage(id)` — image mutations.
